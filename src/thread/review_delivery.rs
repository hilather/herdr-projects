...
fn due(t: &Thread) -> bool {
    if t.status != Status::Open || t.removal.is_some() || t.pending_live_copy.is_some() || t.pending_final_copy.is_some() || t.pending_final_notice.is_some() || t.report_hash.is_empty()
