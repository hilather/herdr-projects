...
fn validate_thread(value:&toml::Value)->Result<()> {
    ...
    for field in ["launch_attempts", "lifecycle_generation", "status_notice_sequence", "review_notice_sequence", "last_review_copy_sequence", "live_copy_sequence", "final_copy_sequence"] { if let Some(v)=value.get(field) { ensure!(v.as_integer().is_some_and(|n|n>=0),"invalid thread integer field"); } }
