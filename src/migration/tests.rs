...
fn task_edits_and_projection_recovery_preserve_new_state_and_originals() {
    ...
    assert!(project.join(format!(".state/projections/schema-11-revision-{head}/TASKS.md")).is_file());
}
#[test]
...
fn published_v2_store_upgrades_explicitly_without_losing_migration_identity() {
    ...
    let after=db.read_snapshot(None).unwrap();assert_eq!(after.tasks,before.tasks);assert_eq!(after.head,before.head);assert_eq!(after.schema_version,11);assert!(db.deliveries().unwrap().is_empty());assert_eq!(db.import_operation_count().unwrap(),0);
