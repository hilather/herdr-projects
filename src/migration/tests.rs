...
#[test]
fn open_active_accepts_sqlite_memory_and_publish_control_marker_preserves_it() {
    ...
    marker.memory="sqlite-v1".into();
    fs::write(project.join(".state/format.json"),serde_json::to_vec_pretty(&marker).unwrap()).unwrap();
    assert!(open_active(&project).is_ok());
    let db=SqliteStore::open(&project.join(".state/state.db")).unwrap();
    publish_control_marker(&project,&db).unwrap();
    let after:Format=serde_json::from_slice(&read(&project.join(".state/format.json")).unwrap()).unwrap();
    assert_eq!(after.memory,"sqlite-v1");
    assert_eq!(after.runtime,"sqlite-v2");
    assert_eq!(after.migration,marker.migration);
    marker.memory="unknown-owner".into();
    fs::write(project.join(".state/format.json"),serde_json::to_vec_pretty(&marker).unwrap()).unwrap();
...
fn task_edits_and_projection_recovery_preserve_new_state_and_originals() {
    ...
    assert!(project.join(format!(".state/projections/schema-{}-revision-{head}/TASKS.md",current.schema_version)).is_file());
}
#[test]
...
fn schema3_inbox_upgrade_reads_provenance_not_changed_legacy_files() {
    ...
    let plan=inspect(&project).unwrap();apply(&project,&plan,true).unwrap();
    let raw=rusqlite::Connection::open(project.join(".state/state.db")).unwrap();raw.execute_batch("DROP TABLE IF EXISTS native_profiles; DROP TABLE IF EXISTS memory_update_receipts; DROP TABLE IF EXISTS memory_delivery_intents; DROP TABLE IF EXISTS memory_import_decisions; DROP TABLE IF EXISTS memory_import_candidates; DROP TABLE IF EXISTS memory_snapshot_inputs; DROP TABLE memory_invalidations; DROP TABLE memory_promotions; DROP TABLE review_decisions; DROP TABLE proposal_validations; DROP TABLE memory_proposals; DROP TABLE coordinator_checkpoints; DROP TABLE coordinator_sessions; DROP TABLE memory_subscriptions; DROP TABLE snapshot_entries; DROP TABLE memory_snapshots; DROP TABLE memory_validity; DROP TABLE memory_dependencies; DROP TABLE memory_heads; DROP TABLE memory_revisions; DROP TABLE memory_records; DROP TABLE objects; DROP TABLE authority_denials; DROP TABLE memory_policies; DROP TABLE routine_occurrences; DROP TABLE routine_cursors; DROP TABLE routine_revisions; DROP TABLE budget_policies; DROP TABLE approval_uses; DROP TABLE approval_revocations; DROP TABLE approval_grants; DROP TRIGGER operation_delivery_monotonic; DROP TABLE attempt_cancellations; DROP TABLE attempt_inputs; DROP TABLE task_dependencies; DROP TABLE task_queue; DROP TABLE scheduler_policy; DROP TABLE runtime_ownership; DROP TABLE project_control; DROP TABLE runtime_observations; DROP TABLE runtime_bindings; DROP TABLE inbox_items; UPDATE store_meta SET schema_version=3; PRAGMA user_version=3;").unwrap();drop(raw);
