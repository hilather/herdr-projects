...
fn reference(value: &VersionedReference) -> bool {
    !value.id.is_empty() && value.id.len() <= 512 && !value.id.chars().any(char::is_control)
        && value.revision > 0 && hash(&value.digest)
}
fn executable(value: &ExecutableIdentity) -> bool {
    std::path::Path::new(&value.path).is_absolute() && value.path.len() <= 4096
        && !value.path.chars().any(char::is_control) && hash(&value.digest)
        && !value.version.is_empty() && value.version.len() <= 96
        && value.version.bytes().all(|b| b.is_ascii_alphanumeric() || b".-+".contains(&b))
}

impl FrozenProfile {
    pub fn validate(&self) -> Result<(), String> {
        if self.version != 1 || !identifier(&self.name) || !identifier(&self.kind)
            || !hash(&self.definition_digest) || !hash(&self.arguments_digest)
            || !std::path::Path::new(&self.config.path).is_absolute() || self.config.path.len() > 4096
            || self.config.path.chars().any(char::is_control)
            || self.config.digest.as_ref().is_some_and(|d| !hash(d))
            || !reference(&self.permission_policy) || !reference(&self.adapter)
            || !executable(&self.agent) || !executable(&self.herdr)
            || self.workflow_certificate.as_ref().is_some_and(|r| !reference(r)) {
            return Err("invalid frozen profile identity".into());
        }
        let mut names = std::collections::BTreeSet::new();
        if self.environment_names.len() > 128 { return Err("too many environment references".into()); }
        for name in &self.environment_names {
            if name.is_empty() || name.len() > 128
                || !(name.as_bytes()[0].is_ascii_alphabetic() || name.starts_with('_'))
                || !name.bytes().all(|b| b.is_ascii_alphanumeric() || b == b'_') || !names.insert(name) {
                return Err("invalid frozen environment reference".into());
            }
        }
        let c = &self.capabilities;
        for capability in [&c.launch, &c.readiness_observation, &c.prompt_submission, &c.stop,
            &c.checkpoint_acknowledgment, &c.structured_usage, &c.resume] {
            match capability {
                CapabilityEvidence::Unknown => {},
                CapabilityEvidence::Supported { evidence } | CapabilityEvidence::Unsupported { evidence }
    ...
    pub fn validate_for_launch(&self) -> Result<(), String> {
        self.validate()?;
        let c = &self.capabilities;
        if [&c.launch, &c.readiness_observation, &c.prompt_submission, &c.stop].iter()
            .any(|c| !matches!(c, CapabilityEvidence::Supported { .. })) {
            return Err("launch requires verified launch, readiness, prompt and stop capabilities".into());
        }
        Ok(())
    }
    ...
    pub fn reference(&self) -> Result<VersionedReference, String> {
        self.validate()?;
        let digest = format!("{:x}", Sha256::digest(serde_json::to_vec(self).map_err(|_| "profile encoding failed")?));
        Ok(VersionedReference { id: format!("profile-{digest}"), revision: 1, digest })
    }
}
...
pub(crate) fn fixture(config: crate::migration::ConfigReference) -> FrozenProfile {
    ...
    FrozenProfile {
        version: 1, name: "fixture".into(), kind: "claude".into(), definition_digest: "b".repeat(64),
        config, arguments_digest: "c".repeat(64), environment_names: vec![],
        permission_policy: evidence.clone(), adapter: evidence,
        agent: ExecutableIdentity { path: "/fixture/claude".into(), digest: "d".repeat(64), version: "1.0.0-preview.1".into() },
...
mod tests {
    ...
    fn unknown_capabilities_remain_explicit_and_identity_covers_effective_inputs() {
        let profile = fixture(crate::migration::ConfigReference { path: "/fixture/config.toml".into(), digest: None });
